...
export class LocalAgentDaemon {
  ...
  async start() {
    ...
    try {
      await this.journal.init(); await this.workspaces.load();
  ...
  async handle(method, params) {
    if (method === 'daemon.status') return { protocol_version: 1, pid: process.pid, platform: process.platform, version: '0.1.0', workspaces: this.workspaces.list(), models: await this.models.list(), runs: [...this.runs.values()] };
    ...
    if (method === 'model.configure') {
      ...
      if (!credential?.refresh_token) throw new Error('local model credentials require device pairing');
      return this.models.set(params.agent_id, { base_url: params.base_url, model_id: params.model_id, api_key: params.api_key }, credential.refresh_token);
    }
  ...
  async executeRun(run, workspace, onEvent = null) {
    ...
    try {
      const credential = await loadCredential(this.paths.dataDir); if (!credential?.refresh_token) throw new Error('local run requires device pairing');
      const model = await this.models.get(run.agent_id, credential.refresh_token); if (!model) throw new Error('no local model credential is configured for this agent');
  ...
  async claimRemoteRun(claim) {
    ...
    const run = { run_id: offer.run_id, lease_id: offer.lease_id, workspace_id: workspace.id, agent_id: offer.agent_id, origin: 'web', sync_mode: 'full', prompt: offer.messages?.at(-1)?.content || '', messages: [{ role: 'system', content: offer.system_prompt }, ...(offer.messages || [])], state: 'running', created_at: new Date().toISOString(), last_sequence: 0 };
