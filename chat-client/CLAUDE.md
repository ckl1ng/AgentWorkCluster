...
- Local Agent 默认执行文本模型运行，另支持 `codex` 执行器（把运行委托给本机 Codex 外部 CLI agent，黑盒、工作区内、内部工具不受平台治理）；尚无本机文件/进程工具、工具确认或断线恢复。帮助内容必须明确此限制，且不得把本机 API Key、Codex 凭据或绝对路径写入示例。
