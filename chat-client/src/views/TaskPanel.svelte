...
<aside class="tasks" class:collapsed aria-label="任务工作台">
  ...
  {#if selected}
    <section class="detail"><div class="detail-heading"><h3>{selected.title}</h3><span>{labels[selected.state] || selected.state}</span></div>
      ...
      <section><h4>提交结果</h4>{#each detail.results as result (result.id)}<p class="event"><small>{new Date(result.created_at).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai', hour12: false })}{result.risk_summary ? ` · 风险：${result.risk_summary}` : ''}</small>{result.result}</p>{:else}<p class="muted">执行者尚未提交结果</p>{/each}</section>
