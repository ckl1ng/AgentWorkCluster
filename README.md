...
| `qq-gateway` | QQ Bot Webhook、事件去重、Token 和 Agent 回复网关 | `9013` | [README](qq-gateway/README.md) |
...
QQ Bot 接入见 [qq-gateway README](qq-gateway/README.md)。公网只暴露 `/qq/webhook*`，Gateway 通过服务密钥调用 Agent 的内部 Channel API；QQ AppSecret 和 Gateway 事件内容均不进入日志。
